...
select yn in "Yes" "No"; do
	case $yn in
		...
		Yes ) ~/rpi/nlp/tensorflow$ unset http_proxy 
		      ~/rpi/nlp/tensorflow$ unset https_proxy 
		      ~/rpi/nlp/tensorflow$ export HTTPS_PROXY=https://username:pass@proxy-server:port
		      ~/rpi/nlp/tensorflow$ export HTTPS_PROXY=https://username:pass@proxy-server:port
			  break;;
		No  ) break;;
	esac
...
select yn in "Yes" "No"; do
	...
	case $yn in
		Yes ) time bazel build --jobs 4 -c opt --copt=-mavx --copt=-mavx2 --copt=-mfma \
				--copt=-msse4.1 --copt=-msse4.2 //tensorflow/tools/pip_package:build_pip_package
			  break;
		# If No AVX2 support, fallback to deafult config options
		No )  bazel build --jobs 4 --config=opt //tensorflow/tools/pip_package:build_pip_package
			  break;
